...
public class MainSceneController : MonoBehaviour
{
    ...
    public Dropdown devicesDropdpdown;
    ...
    void CheckDevices(IRtcEngine engine)
    {
        ...
        string dvName = "defaultName";
        string dvID = "defaultID";
        ...
        for (int i = 0; i < cnt; i++)
        {
            deviceManager.GetVideoDevice(i,ref dvName, ref dvID);
            Debug.Log("Device Name: " + dvName);
            Debug.Log("Device ID: " + dvID);
           
            deviceIDs.Add(dvID);
            deviceNames.Add(dvName);
            
            // if(i==1)
            // {
            //     Debug.Log("Setting camera as logitech");
            //     deviceManager.SetVideoDevice(dvID);
            // }
        }
    ...
    {
        devicesDropdpdown.ClearOptions();
        ...
        devicesDropdpdown.AddOptions(options);         
        devicesDropdpdown.SetValueWithoutNotify(selectedIndex);
    }
    public void SelectDevice(int deviceIndex)
    {
        selectedIndex = deviceIndex;
        List<Dropdown.OptionData> options  = devicesDropdpdown.options;
        ...
        deviceManager.SetVideoDevice(deviceIDs[deviceIndex]);
                

        devicesDropdpdown.SetValueWithoutNotify(deviceIndex);
